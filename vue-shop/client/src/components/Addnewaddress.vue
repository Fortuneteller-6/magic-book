                            ...
                            <div class="module-dialog clear module-dialog-address module-dialog-show">
                                <div class="dialog-panel">
                                    <div class="topbar">
                                        ...
                                        </div><span class="dialog-close" @click.self="hiddenAddAddress">x</span>
                                    </div>
                                                ...
                                                <div class="save-address-box">
                                                    <div class="address-form">
                                                        <div class="module-form-row">
                                                            <div class="form-item-v3"><i> </i><input
                                                                    type="text" class="js-verify" v-model="newAddress.name" placeholder="Receiver Name" >
                                                                <div class="verify-error"></div>
                                                            </div>
                                                        </div>
                                                        <div class="module-form-row">
                                                            <div class="form-item-v3"><i></i><input
                                                                    type="text" class="js-verify" v-model="newAddress.phone" placeholder="Phone Number">
                                                                <div class="verify-error"></div>
                                                            </div>
                                                        </div>
                                                        <div class="module-form-row clear">
                                                            <div
                                                                ...
                                                                <i></i><input type="text"
                                                                    class="js-verify js-address-area-code" v-model="newAddress.areaCode" placeholder="Area Code">
                                                                <div class="verify-error"></div>
                                                            </div>
                                                            <div
                                                                class="form-item-v3 telephone-w fn-right form-valid-item">
                                                                <i></i><input type="text"
                                                                    class="js-verify js-address-telephone" v-model="newAddress.landLine" placeholder="Phone （option）">
                                                                <div class="verify-error"></div>
                                                            </div>
                                                        </div>
                                                        <div class="module-form-row clear">
                                                            <div class="form-item-v3 select-item province-wrapper">
                                                                <select name="province_code"
                                                                    ...
                                                                    <option :value='province.area_id' :key='province.id' v-for='province in ProvinceData'>{{province.area_name}}</option>
                                                            ...
                                                            <div
                                                                ...
                                                                v-model= 'newAddress.cityId'>
                                                                    <option value="0">City</option>
                                                                      <option :value='city.area_id' :key='city.id' v-for='city in cityList'>{{city.area_name}}</option>
                                                                </select></div>
                                                            <div
                                                                class="form-item-v3 select-item district-wrapper fn-right form-focus-item">
                                                                <select class="city select-city js-form-city js-verify"
                                                                v-model= 'newAddress.countyId'>
                                                                    <option value="0">District</option>
                                                                      <option :value='county.area_id' :key='county.id' v-for='county in countyList'>{{county.area_name}}</option>
                                                                </select></div>
                                                        </div>
                                                        <div class="module-form-row">
                                                            <div class="form-item-v3"><i></i><input type="text"
                                                                    class="js-verify"  v-model= 'newAddress.add' placeholder="Detailed address, such as
                                                                    street name .etc">
                                                                <div class="verify-error"></div>
                                                            </div>
                                                        </div>
                                                        <div class="module-form-row fn-clear" ><input type="checkbox"
                                                                class="hide"><span class="blue-checkbox" :class="{'blue-checkbox-on':newAddress.default}" @click="changeCheckbox"></span>Set
                                                            Default
                                                        </div>
                                                        <div
                                                            class="dialog-blue-btn big-main-btn  js-verify-address  " :class="{'disable-btn':isbtn}"  @click="handleCommit">
                                                            <a>Save</a></div>
                                                    </div>
                                                </div>
                                            </div>
...
export default {
    ...
    watch:{
      ...
      'newAddress.provinceId':function(){
          if(this.newAddress.provinceId==0){
              this.newAddress.cityId=0
          }
      },
       'newAddress.cityId':function(){ 
        this.ProvinceData.forEach(item => {
          if(this.newAddress.cityId!=item.area_id){
    ...
    computed:{
...mapState(['isAddress','ProvinceData','isbtn','newAddress']),
...
countyList:function(){
   ...
   this.cityList.forEach(item => {
     ...
     methods:{
        hiddenAddAddress:function(){
       this.$store.commit('hiddenAddAddress')
     },
     ...
     handleCommit:function(){
         ...
         this.$store.commit('addCommit',this.newAddress)
