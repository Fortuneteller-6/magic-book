            ...
            <div class="item-box">
                    <div class="gallery-wrapper">
                        <div class="gallery">
                            <div class="thumbnail">
                                <ul>
                                    <li :key="index" v-for="(item,index) in goodsdefail.goods_images" @click="handleIndex(index)" :class="{on:current_index==index}">
                                        <img :src="item" >
                                    </li>
                    ...
                    <div class="banner">    
                        <div class="sku-custom-title">
                            <div class="params-price">
                                <span><em style="font-size:25px;">$</em><i>{{goodsdefail.price}}</i></span>
                            </div>
                            <div class="params-info">
                                <h4>{{goodsdefail.name}}</h4>
                                <h6>{{goodsdefail.name_title}}</h6>
                            </div>
                        </div>
                            ...
                            <div class="sku-dynamic-params clear">
                                ...
                                <div class="params-detail clear">
                                    <div class="item-num js-select-quantity">
                                        <span class="down" @click="handleDecrease" :class="{'down-disabled':count<2}" >-</span>
                                        <span class="num" >{{count}}</span>
                                        <span class="up" @click="handleIncrease" :class="{'up-disabled':count>goodsdefail.limit_num-1}">+</span>
                                    </div>
                                </div>
                            </div> 
                        </div>
                        <div class="sku-status">
                            <div class="cart-operation-wrapper clearfix">
                                <span class="blue-title-btn js-add-cart" ><a @click="addToCart">Add to Cart</a></span>
                                <span class="gray-title-btn" @click="buyNow(goodsdefail.name)"><a>Buy Now</a></span>
                            </div>
                        </div>
                    </div>
            </div>
...
methods:{
 ...
 var goodsItem={goods:this.goodsdefail,count:this.count}
    this.$store.commit('addCartPanelItem',goodsItem)
   ...
   this.count++
    if(this.count>this.goodsdefail.limit_num){
           this.count =5
        }
   },
    ...
    props:["goods_id"],
    computed:{
        ...mapState(['goodsdefail',] ) },
    mounted(){
    
    
      this.$store.dispatch('getGoodsAsync',this.goods_id)
},
