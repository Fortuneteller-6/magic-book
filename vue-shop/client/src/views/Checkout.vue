                    ...
                    <div class="box-inner js-checkout-address-panel ">
                        <div class="address-common-table js-multiple-address-panel">
                            <ul class="address-item-list clear js-address-item-list">
                                <li class="js-choose-address " :class="{'selected-address-item':item.default}" :key="index" v-for="(item,index) in checkoutdata" :item="item" @click.stop='handlePrevent(item.id)'>
                                    ...
                                    <div class="operation-section">
                                        <span class="update-btn" @click.stop="changeAddress(item.id)">updata</span>
                                        <span class="update-btn" @click.stop="deleteAddress(item.id)">delete</span>
                                    </div>
                                </li>
                             
                                <li class="add-address-item js-add-address">
                                    <p @click='showAddAddress'>Use new Address</p>
                                </li>
                            </ul>
                        </div>
                ...
                <div class="box-inner invoice-box js-invoice-box">
                    ...
                    <div class="invoice-detail"> Invoice Title：
                        <div class="radio-box"><label><input type="radio" class="hide"><span
                                    class="blue-radio  " :class="{'blue-radio-on':Invoice.flag==1}" @click='IntivceL(1)'><a></a></span> personal
                            </label><label><input type="radio" class="hide"><span class="blue-radio" :class="{'blue-radio-on':Invoice.flag==2}" @click='IntivceL(2)' ><a></a></span>
                                company
                            </label></div>
                    </div>
            ...
            <div class="gray-box">
                ...
                <div class="box-inner ui-goods-cart">
                    ...
                    <div class="cart-table">
                        <div class="cart-group js-cart-group">
                            <div class="cart-items" :key="index" v-for="(item,index) in filterChecked">
                                <div class="items-thumb"><a href="javascript:;" target="_blank"><img
                                            :src="item.goods_image"></a></div>
                ...
                <div class="box-inner">
                    <div class="order-discount-line">
                        <p> Total： <span>$ {{checkedAllprice}}</span></p>
                        <p v-if='checkedAllprice>200'> Freight： <span>+ $ 0.00</span></p>
                        <p v-else> Freight： <span>+ $ {{allCheckKind*20}}</span></p>
                ...
                <div class="box-inner">
                    <div class="last-payment clear"><span
                            ...
                            @click="handleSubmitOrder"><a>Submit
                                Order</a></span>
                                <span class="prices fn-right" v-if='checkedAllprice>200' >Sum Payable： <em>$ {{checkedAllprice}}</em></span>
                                <span class="prices fn-right" v-else >Sum Payable： <em>$ {{checkedAllprice+allCheckKind*20}}</em></span>
                            </div>
                </div>
            </div>
        </div>
...
var order={
  orderId:orderId,
  data:year+'-'+month+'-'+day,
  goods:this.filterChecked,
  price:this.checkedAllprice,
...
// console.log(order);
// console.log(this.commitAddress);
this.submitOrder(order)
this.$router.push({name:'payment',query:{orderId:order.orderId}})
  },
  IntivceL(flag){
     this.Invoice.flag=flag
     this.Invoice.name=flag==1?'personal':' company'
  },
  ...mapMutations(['deleteAddress','changeAddress','submitOrder']),
showAddAddress:function(){
this.$store.commit('showAddAddress')

},
handlePrevent:function(id){
  this.$store.commit('checkPrevent',id)
}
},
components:{
    Addnewaddress
...
computed:{
...mapState(['checkoutdata',' cartPanelData','commitAddress']),
...mapGetters(['filterChecked','checkedAllprice','allCheckKind','freightPrice'])
},
mounted(){
  
  this.$store.dispatch('CheckoutAsync',this.user_id);
  this.$store.dispatch('provinceAsync')
},
}
</script>
