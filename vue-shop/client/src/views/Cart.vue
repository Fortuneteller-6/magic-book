                    ...
                    <div v-if='totalNum>0'>
                        ...
                        <div class="cart-table">
                            <div class="cart-group">
                                <div class="cart-top-items" :key="index" v-for='(item,index) in cartPanelData'>
                                    <div class="cart-items">
                                        <div class="items-choose">
                                            <span class="blue-checkbox-new  "  :class="{'checkbox-on':item.checked}" @click="handleChecked(item.id)">
                                                <a></a>
                                            </span>
                                        </div>
                                        ...
                                        <div class="operation">
                                            <a class="items-delete-btn" @click="deleteCart(item.id)"></a>
                                        </div>
                                       <div class="subtotal">$ {{item.price*item.count}}</div>
                                        <div class="item-cols-num">
                                            <div class="select js-select-quantity">
                                                <span class="down " @click='handleDown(item.id)'>-</span>
                                                <span class="num">{{item.count}}</span>
                                                <span class="up" @click='handleUp(item.id)'>+</span>
                                            </div>
                                        </div>
                    ...
                    <div class="cart-bar-operation">
                        <div>
                            <div class="choose-all js-choose-all"><span
                                    class="blue-checkbox-new "  @click="allCheck(cartAllChecked)" :class="{'checkbox-on':cartAllChecked}" ><a></a></span>
                                Check All
                            </div>
                            <div class="delete-choose-goods" @click='deleteAllCheck'>Delete selected items</div>
                        </div>
                    </div>
                    <div class="shipping">
                        <div class="shipping-box">
                            <div class="shipping-total shipping-num">
                                <h4>
                                    Checked <i>{{ allCheckKind}}</i></h4>
                                <h5>
                                    Quantity <i>{{allKindNum}}</i></h5>
                            </div>
                            <div class="shipping-total shipping-price">
                                <h4>
                                    Total payable：<span>$ </span><i> {{checkedAllprice}}</i></h4>
...
methods:{
  handleChecked:function(id){
    this.$store.commit('changeChecked',id)
  },
  handleDown:function(id){
  this.$store.commit('cartHandleDown',id)
  },
    handleUp:function(id){
  this.$store.commit('cartHandleUp',id)
  },
 ...
 this.$store.commit('deleteCart',id)
  ...
  allCheck:function(ischecked){
    this.$store.commit('allChecked',ischecked)
  },
...
computed:{             
  ...mapState(['cartPanelData']),
  ...mapGetters(['cartAllChecked', 'checkedAllprice','allCheckKind','freightPrice','allKindNum','totalNum'])
}}
</script>
